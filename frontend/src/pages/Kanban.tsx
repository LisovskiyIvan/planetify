...
export function Kanban() {
  ...
  useEffect(() => {
    ...
    const fetchBoards = async () => {
      const res = await fetch(`/api/kanban/${userId}/boards`, {
        method: "GET",
        headers: {
          ...
          >
            {selectedBoard && currentBoard ? (
              currentBoard.columns.length > 0 ? (
                ...
                <div className="flex max-h-[30%] content-center w-full lg:w-3/6 flex-col items-center justify-center p-4 sm:p-8 bg-white/20 backdrop-blur-sm rounded-xl shadow-lg text-xl sm:text-3xl text-white">
                  <svg
                    className="w-12 h-12 sm:w-20 sm:h-20 mb-4 text-white/80"
                    fill="none"
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                  ...
                  <p className="text-center">
                    Упс, кажется вы еще не создали колонки!
                  </p>
                  <p className="mt-2 text-sm sm:text-xl text-white/80">
                    Добавьте первую колонку, чтобы начать работу
                  </p>
                </div>
              )
            ) : null}
