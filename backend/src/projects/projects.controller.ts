...
import { CreatePostInput, INewPost, IOldPost } from './projects.model';
...
export class ProjectsController {
  ...
  async getAllProjectsAndPosts(@Param() params: { userId: string }) {
    return await this.projectsService.getUserWithPostsAndProjects(
  ...
  @Put('/')
  async updateProject(
    @Body() { projectId, title }: { projectId: string; title: string },
  ) {
  ...
  @Patch('/post')
  async updatePost(
    @Body() { oldPost, newPost }: { oldPost: IOldPost; newPost: INewPost },
  ) {
